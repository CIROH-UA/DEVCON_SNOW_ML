...
class Region_SWE_Simulation:
    ...
    """ 
    def __init__(self, cwd, datapath, area, year, modeltype):
        self = self
        self.cwd = cwd
        ...
        if not os.path.exists(self.cwd+'//extrapolation//'+area):
            os.makedirs(self.cwd+'//extrapolation//'+area)
            print('Created new directory:', self.cwd+'//extrapolation//'+area)
        if not os.path.exists(self.cwd+'//extrapolation//'+area+'//Predictions'):
            os.makedirs(self.cwd+'//extrapolation//'+area+'//Predictions')
            print('Created new directory:', self.cwd+'//extrapolation//'+area+'//Predictions')
        if not os.path.exists(self.cwd+'//extrapolation//'+area+'//Data//Processed'):
            os.makedirs(self.cwd+'//extrapolation//'+area+'//Data//Processed')
            print('Created new directory:', self.cwd+'//extrapolation//'+area+'//Data//Processed')
        if not os.path.exists(self.cwd+'//extrapolation//'+area+'//Data//NetCDF'):
            os.makedirs(self.cwd+'//extrapolation//'+area+'//Data//NetCDF')
            print('Created new directory:', self.cwd+'//extrapolation//'+area+'//Data//NetCDF')
        if not os.path.exists(self.cwd+'//extrapolation//'+area+'//Data//WBD'):
            os.makedirs(self.cwd+'//extrapolation//'+area+'//Data//WBD')
    ...
    def PreProcess(self, shapefile_path):
        ...
        Geo_df.to_csv(self.cwd+'//extrapolation//'+self.area+'//'+self.area+'_Geo_df.csv', index= True)
    ...
    def Prepare_Prediction(self):
        ...
        submission_format.to_csv(self.cwd+'//extrapolation//'+self.area+'/Predictions/submission_format.csv')

        #also need to save initial 9/24 submission format
        submission_format.to_csv(self.cwd+'//extrapolation//'+self.area+'/Predictions/submission_format_'+str(self.year)+'-09-24.csv')

        print("Prediction CSV Created")

        Geo_df['Region']=""
        print('Defining Encompassing Model Regions')
        National_Snow_Model_Regional.SWE_Prediction(self.cwd, self.datapath, self.area, str(self.year)+'-10-01', self.modeltype, self.year).Region_id(Geo_df)     
        ...
        Region_list = National_Snow_Model_Regional.SWE_Prediction(self.cwd, self.datapath, self.area, str(self.year)+'-10-01', self.modeltype, self.year).Region_list

        init_dict = {}
        Region_Pred={}
        for region in Region_list:
            init_dict[region] = pd.read_hdf(self.cwd+'//extrapolation/Data/Processed/initialize_predictions.h5', key = region)
        ...
        for region in Region_list:
            init_dict[region].to_hdf(self.cwd+'//extrapolation//'+self.area+'//Predictions/predictions'+str(self.year)+'-09-24.h5', key = region)


        o_path=self.cwd+'//extrapolation//'+self.area+'//Data//Processed/Prediction_DF_'+str(self.year)+'-09-24.pkl'
        outfile = bz2.BZ2File(o_path, 'wb')
        pickle.dump(init_dict, outfile)
        outfile.close()

        outfile = self.cwd+'//extrapolation//'+self.area+'//Data//Processed/Region_Pred.pkl'
        ...
        GM_template.to_csv(self.cwd+'//extrapolation/Data/Pre_Processed_DA/ground_measures_features_'+str(self.year)+'-09-24.csv')

        DA_template = pd.read_csv(self.cwd+'//extrapolation/Data/Processed/DA_ground_measures_features_template.csv')
        DA_template['Date'] = str(self.year)+'-09-24'
        DA_template.to_csv(self.cwd+'//extrapolation//'+self.area+'/Data/Processed/DA_ground_measures_features_'+str(self.year)+'-09-24.csv')
    ...
    def Regional_Predict(self, start_date, end_date, day_interval=7, plot =True):
        ...
        for date_ in datelist:
            print('Updating SWE estimates for ', date_)
            ...
            Snow = National_Snow_Model_Regional.SWE_Prediction(self.cwd, self.datapath, self.area, date_, self.modeltype, self.year, day_interval=day_interval)
